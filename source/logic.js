 ...
 * @param {org.acme.shipping.perishable.StartShipment} startShipment - the ShipmentReceived transaction
 * @transaction
 */
function startShipment(startShipment) {
    var shipment = startShipment.shipment;
...
function payOut(shipmentReceived) {
    ...
    if (shipmentReceived.timestamp > contract.arrivalDateTime) {
        ...
        if (shipment.temperatureReadings) {
            ...
            payOut = (contract.unitPrice * shipment.unitCount);
    ...
    contract.importer.accountBalance -= contract.shippingCost;
    contract.shipper.accountBalance+=contract.shippingCost;
 ...
 * @transaction
 */
function locationReading(locationReading) {
    
        var shipment = locationReading.shipment;
...
/**
 * A temperature reading has been received for a shipment
 * @param {org.acme.shipping.perishable.PlaceBid} placeBid - the placebid transaction
 * @transaction
 */
function placeBid(placeBid) {
    
        var contract = placeBid.contract;
        var shipper = placeBid.shipper;
    
        console.log('Adding bid ' + contract.$identifier + ' to shipment ' + shipper.$identifier);
    
        if (contract.bids) {
            contract.bids.push(placeBid.bidvalue+"::"+shipper.$identifier+"::"+Date.now());
        } else {
            contract.bids = [placeBid.bidvalue+"::"+shipper.$identifier+"::"+Date.now()];
        }
    
        return getAssetRegistry('org.acme.shipping.perishable.Contract')
            .then(function (contractRegistry) {
                // add the bidvalue   to the Contract
...
/**
 * A temperature reading has been received for a shipment
 * @param {org.acme.shipping.perishable.AwardContract} awardContract - the placebid transaction
 * @transaction
 */
function awardContract(awardContract) {
    
        var contract = awardContract.contract;
        var shipper = awardContract.shipper;
        var shippingCost=awardContract.shippingCost;
    
        contract.shipper=shipper;
        contract.status='CLOSE';
        contract.shippingCost=shippingCost;
